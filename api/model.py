...
def load_data():
    ...
    df = pd.read_csv(os.path.join(base, "german_credit.csv"))
    return df
...
def get_model():
    global _model, _df_processed, _feature_names
...
def get_portfolio_breakdown():
    ...
    return {
        "total": total,
        "riskBreakdown": [
            {"tier": "Low Risk",    "count": low,    "percentage": round(low/total*100, 1),    "color": "#10b981"},
            {"tier": "Medium Risk", "count": medium, "percentage": round(medium/total*100, 1), "color": "#f59e0b"},
            {"tier": "High Risk",   "count": high,   "percentage": round(high/total*100, 1),   "color": "#ef4444"},
        ]
    }
...
def get_expected_loss():
    ...
    return [
        {"tier": t, "expectedLoss": round(float(np.mean(v)), 2) if v else 0, "color": c}
        for (t, v), c in zip(tiers.items(), ["#10b981", "#f59e0b", "#ef4444"])
    ]
...
def get_loss_distribution():
    ...
    for _ in range(n_simulations):
        ...
        total_loss = float((defaults * LGD * EAD).sum())
    ...
    return {
        "data": [{"simulation": i+1, "loss": round(v, 2)} for i, v in enumerate(sampled)],
        "var95": round(float(np.percentile(portfolio_losses, 95)), 2),
        "var99": round(float(np.percentile(portfolio_losses, 99)), 2),
    }
...
def get_feature_importance():
    ...
    return [{"feature": f, "importance": round(float(v), 4)} for f, v in fi]
...
def get_model_performance():
    ...
    return {
        "models": [
            {"model": "Logistic Regression", "auc": round(float(lr_auc), 3)},
            {"model": "Random Forest",       "auc": round(float(rf_auc), 3)},
        ],
